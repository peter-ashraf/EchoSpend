...
export function VoiceConfirmModal({ isOpen, onClose, data, onConfirm }: VoiceConfirmModalProps) {
  ...
  return (
    <Modal
      ...
      footer={
        <>
          <button type="button" onClick={onClose} className="flex-1 py-3 px-4 rounded-xl border border-neutral-800 text-neutral-300 hover:bg-neutral-900 transition-colors text-sm font-semibold">Cancel</button>
          <button type="button" onClick={handleSave} disabled={items.some(item => !item.amount || parseFloat(item.amount as any) <= 0)} className="flex-1 py-3 px-4 rounded-xl bg-[#0a7ea4] hover:bg-[#086F8A] disabled:opacity-50 text-white transition-all text-sm font-bold flex items-center justify-center gap-2 shadow-lg shadow-[#0a7ea4]/20 active:scale-95">
        ...
        <div className="p-3.5 rounded-2xl bg-[#0a7ea4]/10 border border-[#0a7ea4]/30 flex items-start gap-2.5">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between gap-2 mb-1">
              <p className="text-[11px] uppercase font-bold text-[#0a7ea4] tracking-wider">Voice Input</p>
              {isGemini ? (
                <span className="text-[10px] font-bold text-cyan-300 bg-cyan-950/80 border border-cyan-500/30 px-2 py-0.5 rounded-full flex items-center gap-1 shrink-0"><Sparkle size={10} weight="fill" className="text-cyan-400" /> Gemini AI</span>
              ) : (
                <span className="text-[10px] font-bold text-amber-300 bg-amber-950/80 border border-amber-500/30 px-2 py-0.5 rounded-full flex items-center gap-1 shrink-0">Local Parser</span>
              )}
            </div>
          ...
          <div key={index} className="relative p-4 rounded-2xl bg-neutral-900/50 border border-neutral-800 space-y-4">
            {items.length > 1 && (
              <div className="flex items-center justify-between">
                ...
                <button type="button" onClick={() => handleRemoveItem(index)} className="p-1.5 text-neutral-500 hover:text-red-400 bg-neutral-800 rounded-lg transition-colors"><Trash size={14} weight="bold" /></button>
              </div>
            )}
            <div>
              ...
              <div className="relative">
                ...
                <input type="number" step="any" inputMode="decimal" required value={item.amount || ''} onChange={(e) => handleItemChange(index, 'amount', e.target.value)} placeholder="0.00" className="w-full pl-14 pr-4 py-3 bg-neutral-900 border border-neutral-800 rounded-2xl text-white font-mono text-xl font-extrabold focus:outline-none focus:border-[#0a7ea4]" />
              </div>
            </div>
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-neutral-400 mb-1">Merchant / Description</label>
              <div className="relative">
                <Storefront size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-neutral-500" />
                <input type="text" required value={item.merchant || ''} onChange={(e) => handleItemChange(index, 'merchant', e.target.value)} placeholder="e.g. Starbucks" className="w-full pl-10 pr-4 py-2.5 bg-neutral-900 border border-neutral-800 rounded-xl text-white text-sm focus:outline-none focus:border-[#0a7ea4]" />
              </div>
            </div>
